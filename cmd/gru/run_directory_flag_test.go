...
func TestRunCLIDirectoryFlag(t *testing.T) {
	t.Setenv("LOG_LEVEL", "")
...
`
	...
	cmd := newRootCmd()
	args := []string{"-C", tmp, "run", "--env", "local"}
	cmd.SetArgs(args)
	...
	if wd, err := os.Getwd(); err != nil || wd != tmp {
		t.Fatalf("expected cwd %q, got %q err=%v", tmp, wd, err)
	}
}
